   ...
   const fetchStudentDetails = async () => {
      ...
      try {
         const data = await axios.get(
            `https://intercord-server.vercel.app/api/search/student/${id}`,
            {
               headers: {
   ...
   const handleDeleteStudentAccount = async () => {
      ...
      try {
         const data = await axios.delete(
            `https://intercord-server.vercel.app/api/admin/delete/student/${id}`,
            {
               headers: {
                  Authorization: `Bearer ${user.token}`,
               },
      ...
      <div className="w-full h-fit pl-[1.5rem] sm:pl-[2rem] md:pl-[6rem] pt-[2rem] overflow-y-scroll py-8">
         ...
         <div className="w-full h-fit mt-8">
            <div className="flex max-md:flex-col w-full gap-10 flex-wrap flex-shrink-0">
               {/* Full Name */}
               <div className="flex w-full md:w-[20rem] flex-col font-bona">
                  {!loading ? (
                     <>
                        <label
                           htmlFor="fullname"
